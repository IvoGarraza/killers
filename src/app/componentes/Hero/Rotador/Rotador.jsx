...
const Rotador = () => {
    ...
    return (
        <motion.div
            ref={targetRef}
            style={{ rotate }}
            className="sm:size-44 size-44 z-30 absolute sm:right-24 right-12 sm:-bottom-22 bottom-72 bg-rojo flex items-center justify-center rounded-full p-2"
        >
            {/* Texto circular - solo se renderiza en el cliente */}
            <CircularText text={text} radius={65} />
            
            {/* Centro con cohete */}
            <div 
                style={{
                    background: 'linear-gradient(-45deg, #eae2b7 40%, #fcbf49 50%, #eae2b7 60%)',
                    backgroundSize: '300%',
                    backgroundPositionX: '-150%',
                }}
                className="bg-fondo-claro rounded-full p-8 z-10"
            >
                <Image alt="cohete" className="size-24" src={cohete} />
            </div>
        </motion.div>
    )
}
